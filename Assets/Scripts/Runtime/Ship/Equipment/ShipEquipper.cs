...
namespace NewKris.Runtime.Ship.Equipment {
    public class ShipEquipper : MonoBehaviour {
        ...
        private void ApplyWeapons() {
            try {
                WeaponData weapon1Data = weaponDatabase.weapons.First(x => x.id == EquipmentBlackBoard.weapon1Id);
                ...
                WeaponData weapon2Data = weaponDatabase.weapons.First(x => x.id == EquipmentBlackBoard.weapon2Id);
